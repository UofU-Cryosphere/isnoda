...
#
# Example call:
#   ./net_HRRR_for_topo_SPIReS.sh <YYYY> <MM> <DSWRF_in> <SPIReS_in> <DSWRF_out>
#   ./net_HRRR_for_topo_SPIReS.sh 2021 "0 1 2" /path/to/DSWRF /path/to/SPIReS /path/to/destination
...
export DAY_UTC="${DSWRF_OUT}/net_dswrf.UTC"
...
function net_hrrr_for_month() {
  ...
  echo "  Fetch Albedo"
  SPIReS_MONTH="${SPIReS_IN}/*${MONTH_SELECTOR}*${BASIN}_24.nc"
  SPIReS_MERGE="${DSWRF_OUT}/SPIReS.${MONTH_SELECTOR}.nc"
  echo "${CDO_COMMAND} mergetime ${SPIReS_MONTH} ${SPIReS_MERGE}"
  ${CDO_COMMAND} mergetime ${SPIReS_MONTH} ${SPIReS_MERGE}

  if [[ $? != 0 ]]; then
    echo "  ** Error getting SPIReS Albedo **"
    exit 1
  fi

  echo "  Merge DSWRF and Albedo"
  MERGE_FILE="${DSWRF_OUT}/HRRR_SPIReS.${MONTH_SELECTOR}.nc"
  echo "$CDO_COMMAND merge -selmonth,${MONTH} ${SPIReS_MERGE} -selmonth,${MONTH} ${MONTH_FILE} ${MERGE_FILE}"
  $CDO_COMMAND merge -selmonth,${MONTH} ${SPIReS_MERGE} -selmonth,${MONTH} ${MONTH_FILE} ${MERGE_FILE}

  if [[ $? != 0 ]]; then
    echo "  ** Error merging SPIReS and DSWRF **"
    exit 1
  fi

  echo "  Calculate Net Solar HRRR SPIReS"
  ...
  ${CDO_COMMAND} splitday ${MONTH_CALC_FILE} ${DAY_UTC}.${MONTH_SELECTOR}
