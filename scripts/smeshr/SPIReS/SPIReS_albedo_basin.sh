...
# Script to extract albedo values for a model domain from SPIReS.
...
# Arguments:
# ${1}: /path/to/SPIReS/inputs
# ${2}: water year

set -e

# this is unnecessary, they are already in geotiff
# extract_spires() {
#   # command from UofU snow-rs package
#   # https://github.com/UofU-Cryosphere/snow-rs
#   # Converts the SPIReS matlab to GeoTiff files
#   variable_from_spires --source-dir ${1} \
#                       --water-year ${2} \
#                       --variable albedo_observed_muZ

#   if [ $? != 0 ]; then
#     echo "Error extracting variable from SPIReS inputs"
#     exit 1
#   fi
# }
...
# Extracts the BASIN domain from the Western US SPIReS albedo GeoTiffs
spires_basin() {
  ...
  if [[ -f "${1%.*}_${BASIN}${BASIN_ONE_DAY_SUFFIX}" ]]; then
    ...
    # To extract date for format westernUS_YYYYMMDD_albedo.tif
...
export -f spires_basin
echo "Extracting SPIReS albedo"
${PARALLEL_call} spires_basin {} ::: ${1}/wy${2}/westernUS*albedo.tif
